...
class RedisQueueManager(AbstractQueueManagerServer):
    ...
    def __init__(self, redis_url: str = "redis://localhost:6379", service_name:str="RAG", queue_name: str = "call_agent", priorities:List[str]=["high", "low"]):
    ...
    async def get_data_batch(self, max_batch_size: int = 8, max_wait_time: float = 0.1) -> List[TextFeatures]:
        batch = []
        start_time = time.time()
